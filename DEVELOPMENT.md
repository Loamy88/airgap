...
- [ ] **Feel playtest (human):** walk Blueprint 01 end to end with the Phase 1 controller. Are the corridors the right length? Do the vents shortcut anything worth shortcutting? Does the lighting from Phase 2 read? Fix the drawing, not the code. *(Open `Assets/Scenes/Blueprint01.unity`, hit Play — you spawn in the west yard by the gate.)*
...
- [ ] **Order types as data:** `GuardPoint(anchorId)`, `Patrol(loopId)`, `SetAlertness(level)`, `Investigate`, `HoldPosition`, `Disregard`, `SayAgain`, `WhosNearYou`. Every one maps to a transition the FSM already supports — no order introduces a new guard behavior.
- [ ] **Executability gate:** an order is only *offerable* if the guard can perform it — `Investigate` requires a located event in the memory buffer, `HoldPosition`/`Disregard` require the ladder at Suspicious or Searching, `GuardPoint`/`Patrol` take authored anchor/loop IDs (Phase 3) so no target can be illegal. Un-offerable orders never reach the plausibility check; they're simply absent from the UI.
- [ ] **Plausibility check — `EvaluateOrder(guardId, order) → Obey | Query(reason)`.** A deterministic predicate list, evaluated in order, identical for every caller. **No randomness, no personality input, no knowledge of who transmitted.** Query if any of:
...
- [ ] **Query is an outcome, not a failure mode:** a queried order is *not* executed and emits a `GuardQuery` event carrying the reason code. Phase 7 turns that reason code into a spoken line. The reason code, not the wording, is what the test suite asserts on.
- [ ] **Standing vs. transient orders:** deployment orders overwrite the guard's `standingOrder` (durable). Response orders apply as a `transientOrder` with a TTL, on whose expiry the guard reverts to `standingOrder`. This one distinction is what later makes the spoofer temporary (Phase 10) — build it now, for free, rather than special-casing the spoofer.
- [ ] **Execution latency:** deployment orders queue until the guard finishes their current beat (a few seconds); response orders apply on the next tick.
- [ ] **Validation:** a `-batchmode` suite asserting each predicate fires exactly when it should, that no order path introduces an FSM transition outside the authored set, and that `EvaluateOrder` returns identical results regardless of order source. That last assertion is the spoofer's entire security model, so it's worth a test that fails loudly.
