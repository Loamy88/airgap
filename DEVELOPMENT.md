...
- [ ] Sound event system: emitters (footstep, sprint, pry-bar swing, gadget) broadcast type + loudness + location onto a shared bus. This same bus feeds guards, microphones, and the sensor systems later.
- [ ] **Probabilistic hearing (people) vs. deterministic hearing (machines).** A sound event resolves differently per listener type, and the split is load-bearing:
