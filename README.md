...
### Standard Kit
Three things the Infiltrator always carries, alongside the blueprint above. They cost no gadget slot, have no charges, and are never chosen at setup — they're what you brought because you'd be a fool not to.

They are also, in most situations, **bad ideas.** That's deliberate, and it isn't the same as making them weak. A tool you'd be foolish to use nine times out of ten is a tool you spend the whole round quietly evaluating, and the tenth time — when you've read the room, counted the guards, and *know* — using it feels earned in a way an unconditionally good tool never could. The kit's job is to hand the Infiltrator a permanent, tempting option they must keep talking themselves out of.

- **Flashlight** — a narrow beam, toggled on and off. The facility is genuinely dark, and there are corners of it you cannot read at all without light. But light is the hazard, not the remedy (see Movement & Stance): the beam lifts you out of shadow, drags a moving light pool around with you, and is the most legible thing a guard or camera can possibly catch. A flashlight sweeping across a wall is a *clearer* sighting for a guard than the person holding it would be, and a camera that sees a beam has seen an intruder — no anomaly check required.

  So most of the time, switching it on is simply announcing yourself. The exception is the whole point: a dead-end storeroom, no camera cone on the map, no guard within earshot, nothing to find without light. Then it's just a tool, and using it well means having done the work to be certain first. Off by default.

- **Pry bar** — short, blunt, two-handed, and the tool you reach for when the plan has already gone wrong:
  - **Forcing a door.** Any door that isn't badge-gated can be broken open with repeated hits — no lockpick, no skill check, just time and violence. Each swing is a loud noise event that carries a long way, and each swing **rolls independently** against every guard who might hear it (see Hearing Is a Roll, Not a Radius), so a door that takes five swings is five separate chances to be heard, stacking. A forced door also *stays* visibly forced for the rest of the round, so anyone who walks past it later knows someone came through here.

    It is a last resort and should feel like one: you didn't bring the Lockpick kit, or you brought it and you're out of time, and there's a door between you and the exfil point. Halfway through, having already made three swings' worth of noise, backing out costs you everything you just spent. **Badge-gated doors are immune** — reinforced, no hit count, no exceptions. The vault cluster, the Power Room, and the Ops Room can never be pried, which is what keeps the whole badge economy load-bearing.
...
- **Peeking.** Every exterior entrance authors a **peek aperture** — a dock-door gap, a sidelight window, a grille — and holding Peek at one shows a narrow view of the room behind the door without opening anything. Slow, silent, stationary: the counter to a guard posted just inside an entrance is patience, not a coin-flip. A handful of interior service doors carry apertures too, under one hard rule: **an aperture only ever looks into a plain filler room.** A vault, power, or ops candidate's own door is always blind — reading a prize room takes the Fiber-optic scope (see Found Items). So peeks read the *approach* (anterooms, halls, corridors-behind-doors); the scope reads the *destination*.
...
This is the single rule that gives the loud tools their texture. A pry-bar swing three rooms away is a small chance; the same swing one room away is a large one. Neither is zero and neither is certain, so there is no safe distance to be careless at and no distance where a necessary risk becomes an impossible one. Crucially, **each swing rolls separately** — forcing a door isn't one gamble, it's four or five in a row, and the odds of getting through unheard compound against you the whole time. You always have the option to stop, and you rarely want to, and that's the whole design.

**Machines don't roll.** Microphones, motion sensors, and pressure plates fire deterministically inside their coverage — a sensor either heard you or it didn't, and the Infiltrator can learn exactly where those edges are and route around them. The uncertainty in the game lives in people, and the false-alarm layer (see False Alarms) puts the uncertainty back on the Warden's side of the screen instead. Guards are unpredictable to the Infiltrator; sensors are unreliable to the Warden.
...
No guards patrol outside — the grounds are the safer half of the night, but not a safe one. Exterior security is thin, fixed, and different on every approach:

- **One exterior camera**, mounted over the loading dock — the single outdoor feed on the Warden's Camera Bank, watching the widest way in. Its cone is authored and learnable; going around it is geometry, not luck.
- **One motion floodlight** over the maintenance yard — crossing its zone snaps it on. It sends the Warden nothing but a soft, delayed ping; its real danger is dumping light on you beside an entrance, feeding the same visibility model as every other light.
- **Gravel aprons** at the fence gates and along the service alley — loud footing. Footsteps on gravel emit high-loudness sound events that guards *inside* can roll to hear through the walls. Crouching stays quiet; sprinting across gravel is announcing yourself.

Each approach asks a different question — watched, lit, noisy, or long — and none asks nothing. A handful of **exterior item anchors** are authored into the yards for the found-item pool to occasionally use; nothing mission-critical ever spawns outside, so an exterior find is a reward for the scouting you were doing anyway. (An outdoor guard sweep at high alert remains a possible later escalation — out of scope for v1.)
